...
export function useContentGeneration() {
  ...
  const generatePost = useCallback(async (postData) => {
    ...
    try {
      ...
      if (error.message.includes('QUOTA_EXCEEDED')) {
        toast({
          ...
          description: "You've reached your monthly limit. Upgrade your plan for more content.",
          variant: "destructive",
        });
      } else {
        toast({
  ...
  const generateComment = useCallback(async (commentData) => {
    setIsGenerating(true);
    try {
  ...
  const saveContent = useCallback(async (contentId) => {
    try {
