...
export const CreditTrackingStatus = () => {
  const { 
    subscription, 
    loading, 
    isTrialActive, 
  ...
  const postsPercentage = postsLimit > 0 ? (postsUsed / postsLimit) * 100 : 0;
  ...
  const getPlanDisplayName = () => {
    if (creditData.planName) {
      return creditData.planName;
    }
    return config.name;
  };
  ...
  return (
    <Card className="p-6 bg-gradient-to-br from-slate-50 via-blue-50/30 to-purple-50/30 dark:from-slate-900 dark:via-blue-900/10 dark:to-purple-900/10 border-2 border-slate-200 dark:border-slate-800 shadow-xl">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-600 dark:text-slate-400">
              ...
              {isSubscriptionActive && `Monthly billing - ${formatPrice(creditData.pricing?.total || 0, creditData.pricing?.currency || 'USD')}`}
        ...
        {/* Pricing Display */}
        {isSubscriptionActive && creditData.pricing && (
          <div className="text-right">
            <div className="text-2xl font-bold text-slate-900 dark:text-white">
              {formatPrice(creditData.pricing.total, creditData.pricing.currency)}
            </div>
