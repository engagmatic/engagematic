...
import { validatePersonaCreation, validateObjectId } from '../middleware/validation.js';
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const personas = await Persona.find({ userId, isActive: true }).sort({ createdAt: -1 });
    ...
    const samplePersonas = [
      {
        ...
        description: 'A technology professional with expertise in software development and digital transformation',
        tone: 'professional',
        industry: 'Technology',
        experience: 'senior',
        writingStyle: 'Clear, technical, and solution-oriented. Uses data and examples to support points.'
      },
      {
        name: 'Marketing Expert',
        description: 'A marketing professional focused on growth strategies and brand building',
        tone: 'authentic',
        industry: 'Marketing',
        experience: 'mid',
        writingStyle: 'Engaging, story-driven, and results-focused. Uses personal experiences and case studies.'
      },
      {
        name: 'Entrepreneur',
        description: 'A startup founder sharing lessons learned and business insights',
        tone: 'casual',
        industry: 'Entrepreneurship',
        experience: 'executive',
        writingStyle: 'Conversational, honest, and inspiring. Shares failures and successes openly.'
      },
      {
        name: 'Sales Leader',
        description: 'A sales professional sharing strategies and relationship-building tips',
        tone: 'authoritative',
        industry: 'Sales',
        experience: 'senior',
        writingStyle: 'Direct, persuasive, and value-focused. Uses frameworks and actionable advice.'
      }
    ];
...
// Create new persona
router.post('/', authenticateToken, validatePersonaCreation, async (req, res) => {
  try {
    const userId = req.user._id;
    const { name, description, tone, industry, experience, writingStyle } = req.body;

    // Check persona limit based on plan
    const existingPersonas = await Persona.countDocuments({ userId, isActive: true });
    const personaLimit = req.user.plan === 'pro' ? 10 : 3;

    if (existingPersonas >= personaLimit) {
      return res.status(403).json({
        success: false,
        message: `Persona limit reached. ${req.user.plan === 'starter' ? 'Upgrade to Pro for more personas.' : 'Contact support for assistance.'}`
      });
    }
...
// Set default persona
router.post('/:id/set-default', authenticateToken, validateObjectId, async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user._id;
...
router.delete('/:id', authenticateToken, validateObjectId, async (req, res) => {
  try {
    ...
    const personaCount = await Persona.countDocuments({ userId, isActive: true });
    if (personaCount <= 1) {
      return res.status(400).json({
        success: false,
        message: 'Cannot delete the only persona. Create another persona first.'
      });
    }
