...
router.get('/', async (req, res) => {
  try {
    ...
    const hooks = await Hook.find(query).sort({ usageCount: -1, createdAt: -1 });
...
router.get('/categories', async (req, res) => {
  try {
    const categories = [
      { value: 'story', label: 'Personal Story', description: 'Share personal experiences and lessons' },
      { value: 'question', label: 'Engaging Question', description: 'Ask thought-provoking questions' },
      { value: 'statement', label: 'Bold Statement', description: 'Make confident declarations' },
      { value: 'challenge', label: 'Challenge', description: 'Challenge conventional thinking' },
      { value: 'insight', label: 'Industry Insight', description: 'Share professional insights' }
    ];
...
router.post('/', authenticateToken, checkPlanAccess('pro'), [
  body('text')
    .trim()
  ...
  try {
    ...
    const existingHook = await Hook.findOne({ text: { $regex: new RegExp(text, 'i') } });
    if (existingHook) {
...
router.put('/:id', authenticateToken, checkPlanAccess('pro'), validateObjectId, [
  body('text')
    .trim()
    .isLength({ min: 10, max: 100 })
    .withMessage('Hook text must be between 10 and 100 characters'),
...
router.delete('/:id', authenticateToken, checkPlanAccess('pro'), validateObjectId, async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user._id;
